...
Status player_create(int initial_room_id, int initial_x, int initial_y,
                     Player **player_out)
{
    if (player_out == NULL) return INVALID_ARGUMENT;

    Player *p = malloc(sizeof(Player));
    if (p == NULL) return NO_MEMORY;
...
{
    if (p == NULL || x_out == NULL || y_out == NULL) return INVALID_ARGUMENT;
    *x_out = p->x;
    *y_out = p->y;
    return OK;
}
...
{
    if (p == NULL) return INVALID_ARGUMENT;
    p->x = x;
    p->y = y;
    return OK;
}
...
Status player_try_collect(Player *p, Treasure *treasure)
{
    ...
    if (treasure->collected) return INVALID_ARGUMENT;

    Treasure **new_arr = realloc(p->collected_treasures,
                                 sizeof(Treasure *) *
                                 (size_t)(p->collected_count + 1));
...
{
    if (p == NULL || count_out == NULL) return NULL;
